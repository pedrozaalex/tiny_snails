...
export const urlValidator = string()
  ...
  .required()
  .nullable()
  .test('base url', "you can't create a recursive url, you cheeky bastard", url => !containsBaseUrl(url))

export const snailOwnerValidator = string().trim().nullable();

export const aliasValidator = string()
  .trim()
  .matches(/^[\w-]+$/i, 'slug can only contain letters, numbers, hyphens (-) and underscores (_)')
  .nullable()
  .min(3)
  .max(20)
